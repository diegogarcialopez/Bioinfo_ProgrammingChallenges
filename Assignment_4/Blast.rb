...
def make_db(file, type)                                                                   # Function to create databases
  ...
  if type == Bio::Sequence::NA                                                            # Determining the database type. If the database is a nucleic or protein database
    db_type = 'nucl'
...
def blast(query_id, query_sequence, blast1, blast2, sequences)                                                                       # Function to make the blasts
  ...
  search_results = blast1.query(query_sequence)                                                                                      #  
  if not search_results.hits.empty?                                                                                                  # Checking if there are hits
    ...
    hit_coverage = (best_hit.query_end.to_f - best_hit.query_start.to_f)/best_hit.query_len.to_f
    if best_hit.evalue <= 10**-6 and hit_coverage >= 0.5                                                                             # Filtering for coverage and e-value
      ...
      reciprocal_search = blast2.query(sequences[hit_id])                                                                            # Making the reciprocal blast
      if not reciprocal_search.hits.empty?                                                                                           # Checking if there are hits
        ...
        reciprocal_hit_coverage = (reciprocal_hit.query_end.to_f - reciprocal_hit.query_start.to_f)/(reciprocal_hit.query_len.to_f)
...
def blast_type(file_1, type_1, file_2, type_2)                          # Function to determine the blast type
  ...
  return [search1, search2]                                            # Returning the propoer blast and reciprocal blast according to the databases types
end
...
def search_orthologues(file1, file2)                             # Function to search for the orthologues
  ...
  file1_type = make_db(file1, type1)                             # Creating the database for the first file
  file2_type = make_db(file2, type2)                             # Creating the database for the second file
  searches = blast_type(file1, file1_type, file2, file2_type)    # Determining the blast types
  search1 = searches[0]
  search2 = searches[1]
  sequence2.each do |fasta_seq|
